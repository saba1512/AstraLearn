...
function CoursePage({ copy, courses, language, learning }) {
  ...
  const [lessonVideos, setLessonVideos] = useState(() => {
    const saved = window.localStorage.getItem('astralearn-lesson-videos')
  ...
  const updateLessonVideo = (nextVideo) => {
    setLessonVideos((currentVideos) => {
      ...
      const mergedVideo = { title: '', url: '', ...normalizedCurrentVideo, ...nextVideo }
      ...
      window.localStorage.setItem('astralearn-lesson-videos', JSON.stringify(nextVideos))
      return nextVideos
    })
  }
  ...
  return (
    <section ref={pageRef} className="mx-auto max-w-7xl px-3 py-6 sm:px-6 sm:py-8 lg:px-8">
      <Link
        to="/"
        className="mb-5 inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 transition hover:border-slate-400"
      >
        <ArrowLeft size={17} />
        {copy.back}
      </Link>
      ...
      <div data-gsap="course-block" className="glass-panel overflow-hidden rounded-2xl">
        ...
        <div className="grid min-w-0 gap-6 p-4 sm:p-5 md:p-7 lg:grid-cols-[0.75fr_1.25fr]">
          <div className="min-w-0">
            <p className="text-sm font-black uppercase tracking-wide text-blue-700">{course.badge}</p>
            <h1 className="mt-2 text-2xl font-black leading-tight text-slate-950 sm:text-4xl">
              {course.title}
            </h1>
            <p className="mt-4 max-w-full text-base leading-8 text-slate-600">{course.summary}</p>
      ...
      <div className="mt-6 grid min-w-0 gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <article data-gsap="course-block" className="glass-panel rounded-2xl p-4 sm:p-5 md:p-7">
          <div className="flex items-center gap-3">
            <span className="grid size-11 place-items-center rounded-lg bg-blue-100 text-blue-700">
              <CheckCircle2 size={22} />
            </span>
            <div>
              <p className="text-sm font-black uppercase tracking-wide text-slate-500">{copy.questionLabel}</p>
              <h2 className="text-xl font-black text-slate-950 sm:text-2xl">{lessonWithVideo.title}</h2>
            </div>
          </div>
          <h3 className="mt-6 text-lg font-black leading-8 text-slate-950 sm:text-xl">{lessonWithVideo.question}</h3>
          <p className="mt-4 rounded-lg bg-blue-50 p-4 text-base font-bold leading-8 text-blue-950">
            {lessonWithVideo.answer}
          </p>
          <p className="mt-5 text-base leading-8 text-slate-600">{lessonWithVideo.explanation}</p>
          <p className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm font-bold leading-7 text-amber-900">
