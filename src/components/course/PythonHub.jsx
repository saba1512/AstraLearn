import { ExternalLink, ListChecks, PlayCircle, Terminal, Wrench } from 'lucide-react'
import { getPythonHub } from '../../data/pythonContent'
...
function PythonHub({ language }) {
  const copy = getPythonHub(language)
  ...
  return (
    <section data-gsap="course-block" className="mt-6 glass-panel rounded-2xl p-5 md:p-7">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <ol className="mt-4 grid gap-3">
            {copy.path.map((step, index) => (
              <li key={step} className="flex gap-3 rounded-lg bg-blue-50 p-3">
                <span className="grid size-8 shrink-0 place-items-center rounded-md bg-blue-600 text-sm font-black text-white">
                  {index + 1}
                </span>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <h3 className="flex items-center gap-2 text-xl font-black text-slate-950">
            <Terminal size={21} className="text-emerald-700" />
            {copy.sitesTitle}
          </h3>
              ...
              >
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="rounded-md border border-orange-200 bg-orange-50 px-2 py-1 text-xs font-black text-orange-800">
                      {site.level}
                    </span>
                  </div>
